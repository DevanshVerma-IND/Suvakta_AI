...
// MediaPipe's WASM runtime emits this normal CPU-initialization message through
// console.error. Next.js shows every console.error as a development overlay.
// Filter only this known informational message; real MediaPipe errors remain visible.
if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
  const browserWindow = window as Window & { __speakforgeMediapipeLogFilter?: boolean };
  if (!browserWindow.__speakforgeMediapipeLogFilter) {
    const originalConsoleError = console.error;
    console.error = (...args: unknown[]) => {
      if (args.some((arg) => typeof arg === "string" && arg.includes("Created TensorFlow Lite XNNPACK delegate for CPU"))) return;
      originalConsoleError(...args);
    };
    browserWindow.__speakforgeMediapipeLogFilter = true;
  }
}
...
interface SegmentResult {
  ...
  fillerCount: number; durationSecs: number; avgMetrics: MetricsSnapshot;
}
...
export default function PracticePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Keep the downloaded WASM runtime in sync with the npm package version.
        ...
        console.error("Unable to start pose detection:", error);
      }
    })();
    ...
    return () => {
      ...
      if (poseRef.current === landmarker) poseRef.current = null;
      landmarker?.close();
    };
  }, []);
  ...
  useEffect(() => {
    const loop = () => {
      ...
      if (video && pose && video.readyState >= 2 && now - lastPoseTimeRef.current >= 200) {
        ...
        const t = now;
        let lm: ReturnType<PoseLandmarker["detectForVideo"]>["landmarks"][number] | undefined;
        try {
          lm = pose.detectForVideo(video, t).landmarks?.[0];
        } catch (error) {
          // A browser can briefly expose a stale video frame while the camera is starting.
          // Skip that frame instead of breaking the recording screen.
          void error;
        }
        if (lm) {
  ...
  const startRecording = () => {
    ...
    setIsRecording(true);
    try { recRef.current.start(); } catch { isRecordingRef.current = false; setIsRecording(false); return; }
    setPhase("record");
  };
  ...
  const stopRecording = () => {
    ...
    const result: SegmentResult = {
      ...
      durationSecs: sessionStartRef.current ? (Date.now() - sessionStartRef.current) / 1000 : 0,
      avgMetrics: {
        wpm: hist.length ? Math.round(avg(hist.map(h => h.wpm))) : metricsRef.current.wpm,
  ...
  const nextSegment = (latestResults: SegmentResult[]) => {
    ...
    if (currentSegmentIdx + 1 >= segments.length) {
      ...
      fetch("/api/sessions", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          durationSecs: Math.round(latestResults.reduce((total, result) => total + result.durationSecs, 0)),
  ...
  const startCustomRecording = () => {
    ...
    setCustomDone(false);
    setIsRecording(true);
    try { recRef.current.start(); } catch { isRecordingRef.current = false; setIsRecording(false); }
  };
