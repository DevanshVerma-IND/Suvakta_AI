...
export default function CoachPage() {
  ...
  const askCoach = async () => {
    ...
    try {
      ...
      const metricsWithoutWpm = {
        fillerCount: metricsRef.current.fillerCount,
        gestureEnergy: metricsRef.current.gestureEnergy,
        postureScore: metricsRef.current.postureScore,
        volumeLevel: metricsRef.current.volumeLevel,
        energyScore: metricsRef.current.energyScore,
        variationScore: metricsRef.current.variationScore,
      };
      ...
      if (!res.ok) {
        ...
        setFeedbackError(
          payload?.error === "Not authenticated"
            ? "Please sign in to use live coaching."
            : "Coach unavailable — try again shortly."
        );
        return;
      }
