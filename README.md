...
Suvakta is a real-time AI public speaking coach built with Next.js, MediaPipe, and the Web Audio API. It tracks posture, gestures, vocal energy, pacing, and filler words while you speak — and gives live AI feedback every 5 seconds powered by Gemini.
...
| AI coaching | Gemini 3.6 Flash (`generateContent`) |
...
2. Copy `.env.example` to `.env.local`, then set all four values with real credentials. The committed `.env.local` values are placeholders and will not work.
3. Start the app with `npm run dev`, then open `http://localhost:3000`.

MongoDB stores user accounts and saved sessions. Gemini powers live feedback, summaries, and generated practice sessions. ElevenLabs is used for the optional coach voice in Practice mode.

Camera and microphone access require `localhost` during development or HTTPS when deployed. Speech transcription uses the browser Web Speech API; Chromium-based browsers provide the most complete support.
